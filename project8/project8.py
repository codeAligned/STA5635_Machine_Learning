...
class logisticRegression:
    def __init__(self, xTr, yTr, xVal, yVal,
            nIter=300, eta=0.0025, k=10, s=0.001, miu=100.):
        self.xTr=xTr
        self.yTr=yTr
        self.xVal=xVal
        self.yVal=yVal
    ...
    def train(self):
        ...
        misclassTr=np.zeros(4)
        misclassVal=np.zeros(4)
        betaAll=np.zeros((self.M, 1))
        ik=1
        for i in range(self.nIter):
            H=np.dot(self.xTr, betaAll)
            p=1./(1.+np.exp(-2.*H))
            w=p*p
            temp=(0.5*(self.yTr+1)-p)
            z=np.divide(temp, w, out=np.zeros_like(temp), where=w!=0.)
            ...
            for j in range(1, self.M):
                ...
                if temp==0.:
                    ...
                    betaJ=1./temp*np.array([[c*d-b*e],[a*e-b*d]])
